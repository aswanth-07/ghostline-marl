...
`scripts/qa_scaled_visuals.py` freezes representative title, briefing, Field Manual, pause, debrief, settings, Accessibility, Agent Lab selection/live, and tier-6 gameplay frames, presents each through the shipping renderer at 1280x720 and 1920x1080, and pixel-compares the result with exact 2x/3x nearest-neighbour expansion. The final 2026-07-13 v4 matrix under `artifacts/visual-qa/flat-vision-grades-v4/` recorded zero mismatched pixels and no crop, stretch, or letterbox across all 20 captures. Visual inspection verified the flat 2D opening language, fully readable furniture, Standard/Interceptor/Elite legend and badges, wrapped dossier copy, live telemetry, full `INTEGRITY` HUD label, minimap, smooth 65-ray occlusion-correct patterned vision cones, exposure meter, and color-plus-shape danger cues. The measured hidden-surface full-frame path was 81.85 FPS at 720p and 80.44 FPS at 1080p. Reviewed gameplay and locomotion captures are tracked under `assets/screenshots/`; visible-window and Chrome checks remain separate release gates.
