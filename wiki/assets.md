...
Default hazards use amber for suspicion and red for confirmed danger. Electronics — response drones, pulse, and the suppressor telegraph — use a blue-violet that is separable from danger in every colour mode. The earlier violet differed from danger red only in its blue channel, so all hostile cues collapsed into one pink and Color-Safe then remapped red onto that same magenta; the amber-suspicion/red-danger hazard language itself is unchanged. Color-safe mode remaps danger to pink and extraction/success to blue while preserving distinct symbols, outlines, captions, and motion-independent shapes. High-contrast mode expands luminance separation after composition. Reduced Motion disables camera look-ahead, shake, afterimages, and moving menu art; Reduced Flashes removes pulsing lockdown intensity and limits event particles.
